...
const unauth = {
   ...
   actions: {
      ...
      submitTranscription(ctx, transcription) {
         ctx.commit("setTranscribeError", "")
         if (ctx.state.transcribeFile == null) {
            ctx.commit("setTranscribeError", "File is missing")
            return
         }
         axios.post("/api/transcription", {fileID: ctx.state.transcribeFile.id, transcription: transcription}).then((_response) => {
            ctx.commit("transcribeFile", null) 
         }).catch ( err => {
